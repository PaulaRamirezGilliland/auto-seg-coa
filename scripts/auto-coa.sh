...
echo "-----------------------------------------------------------------------------------------------------------------"
echo
echo "Aorta segmentation for fetal MRI (KCL): auto segmentation of fetal 3D SVR T2w heart recons for CoA risk score analysis"
echo
echo "-----------------------------------------------------------------------------------------------------------------"
echo "-----------------------------------------------------------------------------------------------------------------"
echo
...
if [[ $# -ne 2 ]] ; then
    echo "Usage: bash /home/auto-seg-coa/auto-coa.sh"
...
echo
echo "-----------------------------------------------------------------------------"
echo " REMOVING NAN & RESCALING INTENSITIES "
echo "-----------------------------------------------------------------------------"
echo
...
echo "-----------------------------------------------------------------------------"
echo
echo "3D AORTIC ARCH VESSEL SEGMENTATION OF 3D SVR T2W RECONS..."
echo
...
python3 ${segm_path}/src/run_monai_thorax_loc.py ${main_dir}/ ${monai_check_path}/ stack-info.json ${main_dir}/monai-segmentation-results-thorax ${monai_lab_num}
...
mkdir monai-segmentation-results-coa

python3 ${segm_path}/src/run_monai_atunet_coa.py ${main_dir}/ ${monai_check_path}/ masked-stack-info.json ${main_dir}/monai-segmentation-results-coa ${monai_lab_num}

number_of_stacks=$(find monai-segmentation-results-coa/ -name "*.nii*" | wc -l)
...
out_mask_names=$(ls monai-segmentation-results-coa/cnn-*.nii*)
IFS=$'\n' read -rd '' -a all_masks <<<"$out_mask_names"
...
do
    ...
    ${mirtk_path}/mirtk transform-and-rename ${all_stacks[$i]} ${all_masks[$i]} "-seg-coa-ssm" ${main_dir}/cardiac-masks


done


number_of_final_files=$(ls ${main_dir}/cardiac-masks/*-seg-coa-ssm.nii* | wc -l)
if [[ ${number_of_final_files} -ne 0 ]];then

    cp -r cardiac-masks/*-seg-coa-ssm.nii* ${output_main_folder}/
